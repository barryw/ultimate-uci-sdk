...
Clearing the request makes every optional pointer and length zero. The first
command therefore has no arguments, payload or status buffer. The second adds
the optional GET\_TIME format byte. The hardware-info command is then sent
without its selector and with an explicit model selector. Set payload, data or
status spans in the same pointer-plus-length form only when needed.
