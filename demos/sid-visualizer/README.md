...
frequency and higher envelope energy. Bar height follows the same gate-driven
software envelope and SID master volume.

The assembly playback wrapper snapshots the exact SID values for each 100 Hz
music tick before the player writes them. SID frequency registers are
write-only, so reading `$D400` after playback would not recover the notes. A
30-byte zero-page swap isolates the player locations shared with cc65 and the
SDK. The CIA interrupt keeps playback at 100 Hz while the main loop updates
PETSCII bar heights and palette colours during vertical blank.

The demo requires **U64 Specific Settings → Turbo Control → U64 Turbo
Registers**. It runs at the portable 4 MHz speed so all six meters can refresh
once per video frame, then restores the speed that was active on entry.
...
palette, silences both SIDs, and resets the C64. Use `A`/`D` or the horizontal
cursor keys to switch among the seven songs. Choose the initial song at build
time with `make TUNE=0` through `make TUNE=6`.
...
It temporarily enables the command interface and turbo registers, maps
physical SID socket 2 to `$D500`, proves that playback and PETSCII bar heights
advance at video-frame speed, tests `A`/`D` song selection, and restores all
settings even if the test fails.
